...
// To get the absolute coordinates, it adjusts the given x and y coordinates from the model's coordinate system to the screen's coordinate system. 
// It does so recursively by calling the parent widget's ModelToScreen method, if it exists. Get the offset of the parent and add it to the current widget's coordinates.
...
void Widget::OnMouseDown(int32_t x, int32_t y, common::uint8_t button)
{
    if(Focussable)
        GetFocus(this);
}

void Widget::OnMouseUp(int32_t x, int32_t y, common::uint8_t button)
{
}
